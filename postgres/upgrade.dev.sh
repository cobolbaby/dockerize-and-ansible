...
# 数据库停服，但停服前需确认哪台是 Leader 节点，最好保证可以最后停
# TODO:具体操作此处省略 ...

PGDATA_DEFAULT_TABLESPACE=/data/hdd/pg
...
# 重启一下，确保以 Leader 角色优雅退出
docker run -d --rm --name pg1217 \
	-e PGDATA=/var/lib/postgresql/12/data \
	-v ${PGDATA_DEFAULT_TABLESPACE}/12/data:/var/lib/postgresql/12/data \
    -v ${PGDATA_SECONDARY_TABLESPACE}/12/data:/data/postgresql/12/data \
    registry.inventec/infra/postgres:12.17

docker exec -ti pg1217 pg_ctl stop
